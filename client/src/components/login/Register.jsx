...
function Register(props) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const createPaymentIntent = await axios.get(`${BACKEND_URL}/user/register-payment-intent`);
            console.log("Create Payment Intent ", createPaymentIntent);


            // CONFIRM PAYMENT ON THE CLIENT
            const { error: stripeError, paymentIntent } = await stripe.confirmCardPayment(createPaymentIntent.data.clientSecret, {
                payment_method: {
            ...
            console.log(error);
            dispatch(openModal({ heading: "Payment Incomplete", body: error.message }));
            return;
        }
    ...
    return (
        <Box component='form' sx={{ backgroundColor: '#24242a' }} onSubmit={handleSubmit}>
            ...
            <Typography variant='h6' color='error' >Register with $50 to upload your screenplay</Typography>
            ...
            <Box sx={{ display: 'flex', marginBottom: 2, alignItems: 'space-between', flexDirection: { xs: 'column', md: 'row' } }}>
            ...
            {/* STRIPE ELEMENT  */}
            <CardElement options={cardElementOptions} style={{ border: "1px solid blue" }} />
